...
def ShowAbout():
    ...
    Img_StudioLogo = ImageTk.PhotoImage(Image.open("StudioLogo.png").resize((800,300),Image.LANCZOS))
...
def ShowWeightSettings():
    print(f'{__file__[0:0-(1+len(os.path.basename(__file__)))]}/config.txt')
    os.system(f'start notepad {__file__[0:0-(1+len(os.path.basename(__file__)))]}/config.txt')
...
def ChooseOne():
    ...
    if r.randint(1,200)==1:
        ...
        if len(Chosen) < stu_Quantity:
            ...
            Chosen.append(Tar)
            Lab_Number.config(text=str(Tar),fg=MainTitle_color,font=("MiSans VF Bold",160))
    ...
    if r.randint(1,200)==1:
        ...
        if len(Chosen) < stu_Quantity-int(Ent_N.get())+1:
            ...
            for i in range(int(Ent_N.get())):
                ...
                Lab_Chosen.config(text=str(f'已抽{len(Chosen)}个学生：\n{str(Chosen)}'))
                res+=str(Tar)+' '
            Lab_Number.config(text=str(res),font=("MiSans VF Bold",120),fg=MainTitle_color)
        else:
